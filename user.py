...
form_class = uic.loadUiType("user_client.ui")[0]
...
class WindowClass(QMainWindow, form_class) :
    def __init__(self) :
        ...
        self.pushButton_9.clicked.connect(lambda: self.stackedWidget.setCurrentIndex(0))    #오늘의 운동창에서 홈으로
        ...
        self.pose_test.clicked.connect(self.pose_reform)        #실시간 자세 교정
        self.today_exer.clicked.connect(self.excercise)         #운동창으로 이동
...
# 실시간 자세 교정 쓰레드
    def pose_reform(self):
        while True:
            ...
            th = threading.Thread(target=self.pose_reform_1)
            th.start()


# 실시간 자세 교정
    def pose_reform_1(self):
        # 자세교정 알림 멘트
        def sound_notice():
        ...
        while True:
            ...
            if prediction[0, 0] < prediction[0, 2] or prediction[0, 1] < prediction[0, 3]:
                ...
                try:
                    if wrong_pose % 15 == 0:
                        ...
                        print("거북이 됩니다 그러다")
                        sound_notice()
                        #오자세 알림이 울릴때마다 서버로 신호를 보냄
                        self.sock.send('pose/bad_form'.encode())
            ...
            #타임 슬립사용시 웹캠상태가 원활하지 못하는 문제가 있음
            # time.sleep(1)
